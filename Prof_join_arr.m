function p=Prof_join_arr(parr, keepshape)
% function p=Prof_join_arr(parr, keepshape)
%
...
%
% If keepshape is defined it will keep the shape of "parr" in the fields of "p" (in the case that "parr" has dimension higher than 1).
%
...
for iff=1:length(fnames1) % loop over fields
  ...
  if(nargin()==2) 
    if(keepshape)
      % reshape field to the shape of the original array of structures
      shp = size(f0);
      pshp= size(parr);

      nshp = [shp(1:end-1) pshp];
